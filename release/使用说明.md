...
| --- | --- | --- |
| 主操作 | 隐藏 / 显示 OSD | 开关叠加层 |
| 二级界面 | 位置 → 四角 | 把面板钉到屏幕某个角 |
| 二级界面 | 小图标位置 → 显示在托盘常显区 / 收进托盘溢出区 | 控制小图标显示在**任务栏右侧常显区**（时钟旁边直接可见），还是收进 **`^` 溢出区**（默认） |
| 二级界面 | 背景透明度… | 弹出滑杆，实时调节面板背景不透明度（0% 纯文字 ~ 100% 全不透明） |
| 开关 | 鼠标穿透（勾选） | 面板不挡游戏鼠标操作，默认开启 |
| 开关 | 强制顶层显示（勾选） | 每秒重新抢占一次 Z 序，防止被其他"同样置顶"的窗口压住；默认关闭 |
| 开关 | **开机自启（勾选）** | 登录 Windows 时自动启动（本组最后一项） |
| 工具 | 导出传感器清单 | 把本机所有可用传感器导出到 `%TEMP%\statsosd-sensors.txt`（排查"有没有功耗/每核心数据"用） |
| 工具 | 重启资源管理器 | 任务栏会闪一下；用于让"托盘常显/溢出"的设置确实生效 |
| 工具 | 打开任务栏设置 | 打开 Windows 的 `ms-settings:taskbar`，可手动调整托盘图标 |
| 权限与退出 | 以管理员身份重新启动 | 一键提升权限重启，读取完整传感器 |
